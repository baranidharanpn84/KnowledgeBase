...
dfparquetGold = dfparquet.withColumn("gold_date_created_utc",sf.regexp_replace(dfparquet.createdDate,"T"," ").substr(1, 7))
...
dfparquetGoldStg = dfparquetGold.groupBy(dfparquetGold.gold_date_created_utc).agg(countDistinct("id").alias("gold_cnt_id")).orderBy(sf.asc("gold_date_created_utc"))
