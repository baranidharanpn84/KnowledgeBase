...
dfjoin = dfbaseData.join(dfrmi,(dfbaseData.mvpLoanApplicationId == dfrmi.loan_application_id) & \
                    ...
                    ,sf.regexp_replace(dfbaseData.createdDate,"T"," ").cast(TimestampType()).alias("ewsVerifyIdentityTimestampUTC") \
                    ,dfbaseData.createdDatePT.alias("ewsVerifyIdentityTimestampPT") \
...
dfrankedId =  dfjoin.withColumn("row_num", sf.row_number().over(Window.partitionBy("mvpLoanApplicationId","ewsVerifyIdentityTimestampPT").orderBy(sf.desc("riskTimestampPT"),sf.desc("scoreType")))) \
...
dfsplitColDataLookup = dfbaseData.select(sf.col("id")
                                    ,sf.col("matchResults_result_1_match")
                                    ,sf.col("matchResults_result_1_type")
                                    ,sf.col("matchResults_result_2_match")
                                    ,sf.col("matchResults_result_2_type")
                                    ,sf.col("matchResults_result_3_match")
                                    ,sf.col("matchResults_result_3_type")
                                    ,sf.col("matchResults_result_4_match")
                                    ,sf.col("matchResults_result_4_type")
                                    ,sf.col("matchResults_result_5_match")
                                    ,sf.col("matchResults_result_5_type")
                                    ,sf.col("matchResults_result_6_match")
                                    ,sf.col("matchResults_result_6_type")
                                    ,sf.col("matchResults_result_7_match")
                                    ,sf.col("matchResults_result_7_type")
                                    ,sf.col("matchResults_result_8_match")
                                    ,sf.col("matchResults_result_8_type")
                                    # ,sf.col("matchResults_result_match")
                                    # ,sf.col("matchResults_result_type")
                                    ,sf.col("year")
                                    ,sf.col("month")
                                    ,sf.col("day"))
